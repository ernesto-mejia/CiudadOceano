...
class ExhibitionController extends Controller
{
    ...
    public function getHome($status)
    {
        ...
        switch ($status) {
            ...
            case '1':
                $products = Article::where('status', '1')->where('module', 'exhibiciones')->orderBy('id', 'DESC')->paginate(20);
                break;
            case '0':
                $products = Article::where('status', '0')->where('module', 'exhibiciones')->orderBy('id', 'DESC')->paginate(20);
                break;
            case 'trash':
                $products = Article::onlyTrashed()->where('module', 'exhibiciones')->orderBy('id', 'DESC')->paginate(20);
                break;
    ...
    public function getArticleDelete($id)
    {
        ...
        if($product->delete()):
            ...
            return back()->with('message', ' Exhibición enviada con éxito a la papeplera.')->with('typealert', 'success');
    ...
    public function getArticleRestore($id)
    {
        ...
        if ($product->save()):
            ...
            return redirect('/admin/exhibition/'.$product->id.'/edit')->with('message', ' La exhibición se restauro correctamente.')->with('typealert', 'success')->withInput();
        endif;
